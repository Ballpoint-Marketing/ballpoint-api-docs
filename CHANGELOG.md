...
- **Superseded clarification - recipient dedupe scope (docs-only, no behavior change)** - This v1.3.7 entry reflected the prior `Deliver To = both` wording and is superseded by v1.5.1 above. Current contract: Ballpoint does **not** perform intra-order recipient dedupe; duplicate recipient records uploaded to the same order are treated as separate recipient records unless normal validation rejects the request. If `Deliver To = both` + `Remove duplicates = OFF`, same-lead `property == mailing` may be uploaded as two recipient records and Ballpoint mails two pieces. Same-order collapse happens only when the user explicitly enables Remove duplicates and the partner uploads the matching deduplicated recipient list.
